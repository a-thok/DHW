...
export default function DetailCtrl($http, $stateParams) {
  ...
  $http.post('/Sys/o2o/Product/get', { id: id }).success((data) => {
    vm.data = data.result;
    vm.setSelectedItems(vm.data.sellprov);
    ...
    if (data.result.state === 0 || data.result.state === 1 || data.result.state === 9 || data.result.state === 8) {
      vm.isDisabled = false;
    } else {
      vm.isDisabled = true;
    }
  ...
  vm.submit = function () {
    ...
    // 修改需提交的省份数据结构
    vm.data.sellprov = []; // 先清空原始数据
    const selectedItems = vm.getSelectedItems();
    selectedItems.forEach((item) => {
      vm.data.sellprov.push(item.code); // 推进修改后的code
    });
