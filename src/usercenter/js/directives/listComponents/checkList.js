...
export default function checkList() {
  return {
    ...
    controller: ['$http', '$attrs', '$window', '$scope', function Ctrl($http, $attrs, $window, $scope) {
      let vm = this;
      let params;
      let para;
      ...
      let getData = (pageIndex) => {
        $http.post($attrs.api, Object.assign({}, {
          pageIndex,
          pageSize: 5,
          id: parseInt($scope.$parent[$attrs.vm].id, 10),
        }, params)).success(res => {
      ...
      vm.checked = (event) => {
        ...
        for (let i = 0, len = checkbox.length; i < len; i++) {
          if ($(elem).is(':checked')) {
            ...
            arrid.push(parseInt(checkbox[i].getAttribute('data-id'), 10));
          } else {
