    ...
    controller: ['$scope', '$attrs', function Ctrl($scope, $attrs) {
     ...
     // 根据起始年份来获得结束年份
      vm.getEndYear = () => {
        var today = new Date();
        var thisYear = today.getFullYear();
        for (var arr = [], i = parseInt($scope.$parent[$attrs.vm].startyear, 10); i <= thisYear; i++) {
          arr.push(i + '');
        }
        return arr;
      };
      ...
      vm.getEndMonth = (startYear, endYear, endYearTemp) => {
        if (startYear && (endYear || endYearTemp)) {
          if (startYear == endYear) {
            for (var arr = [], i = parseInt($scope.$parent[$attrs.vm].startmonth, 10); i < 12; i++) {
              arr.push(i < 10 ? '0' + i : i);
            }
            return arr;
          } else {
