  ...
  return {
    ...
    controller: ['$scope', '$attrs', '$http', '$stateParams', function Ctrl($scope, $attrs, $http, $stateParams) {
      ...
      $http.post('/Dict/city').success((res) => {
        ...
        vm.provinces = vm.areaData.filter((item) => item.type === 'province');
      });
      // 点击省获取市
      vm.getCities = (province) => {
        vm.cities = vm.areaData.filter((item) => (
          item.type === 'city' && item.code.slice(0, 2) === province.code.slice(0, 2)
        ));
      ...
      vm.getDistricts = (city) => {
        vm.districts = vm.areaData.filter((item) => (
          item.type === 'district' && item.code.slice(0, 4) === city.code.slice(0, 4)
        ));
